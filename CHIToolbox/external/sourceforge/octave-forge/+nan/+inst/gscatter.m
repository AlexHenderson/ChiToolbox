...
for k=1:length(b);
	...
	hh(k) = plot(x(k==j),y(k==j),'linestyle','none','color',c,'marker',s,'markersize',15);
...
if ~strcmpi(doleg,'off')
	...
	legend(vertcat(text_strings',b), 'location','best','interpreter', 'none');
end;
if ~isempty(xname)
	xlabel(xname); 
end; 
