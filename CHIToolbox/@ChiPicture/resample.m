...
% resample  Reduces the spatial resolution of the picture. 
%
% Syntax
...
%
% Description
%   resample(combine_y,combine_x) combines the pixels in each of combine_y
...
% 
%   obj = resample(____) clones the picture and resamples the clone. The
%   original picture is untouched. 
%
% Example
%   new = orig.resample(2,4,'mean') clones the orig ChiPicture to produce a
...
if nargout
    ...
    this.data = reshape(this.data,pixelstosum_y,[]);
    this.data = feval(functiontoapply,this.data);
    this.data = reshape(this.data,outputrows,[]);

    this.data = this.data';

    this.data = reshape(this.data,pixelstosum_x,[]);
    this.data = feval(functiontoapply,this.data);
    this.data = reshape(this.data,outputcols,[]);
